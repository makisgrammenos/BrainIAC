...
import wandb
from adni_dataset import ADNIDataset, get_validation_transform,get_default_transform
...
class AmyloidClassificationLightningModule(pl.LightningModule):
...
class AmyloidDataModule(pl.LightningDataModule):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(
        project=config['logger'].get('project_name_amyloid', 'amyloid_classification_project'),
        name=config['logger'].get('run_name_amyloid', 'amyloid_finetune_run'),
        config=config
    )

    data_module = AmyloidDataModule(config)
    model = AmyloidClassificationLightningModule(config)
